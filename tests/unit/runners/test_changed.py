...
class TestChanged():
    ...
    @patch('srv.modules.runners.changed.log')
    def test_write_checksum(self, log_mock):
    ...
    def test_read_checksum(self, open_mock, log_mock):
        fs.CreateFile("{}/{}".format(checksum_dir, 'rgw.conf'), contents="foo=bar") 
        cfg = changed.Config('rgw')
    ...
    @patch('srv.modules.runners.changed.log')
    def test_read_checksum_no_file(self, log_mock):
        cfg = changed.Config('rgw')
        ret = cfg.read_checksum()
        log_mock.debug.assert_called_once()
        assert ret is None
    ...
    @patch('srv.modules.runners.changed.log')
    def test_has_changes_not_eq(self, log_mock, write_cs_mock, read_cs_mock, create_cs_mock):
        cfg = changed.Config('rgw')
    ...
    @patch('srv.modules.runners.changed.Config.read_checksum')
    @patch('srv.modules.runners.changed.log')
    def test_has_changes_no_current_cs(self, log_mock, read_cs_mock, create_cs_mock):
        cfg = changed.Config('rgw')
        read_cs_mock.return_value = "0b0b"
    ...
    def test_rgw(self, rcc_mock):
        changed.rgw()
        rcc_mock.assert_called_with('rgw') 

    @patch('srv.modules.runners.changed.requires_conf_change')
    def test_mds(self, rcc_mock):
        changed.mds()
        rcc_mock.assert_called_with('mds') 

    @patch('srv.modules.runners.changed.requires_conf_change')
    def test_osd(self, rcc_mock):
        changed.osd()
        rcc_mock.assert_called_with('osd') 

    @patch('srv.modules.runners.changed.requires_conf_change')
    def test_mon(self, rcc_mock):
        changed.mon()
        rcc_mock.assert_called_with('mon') 

    @patch('srv.modules.runners.changed.requires_conf_change')
    def test_global_(self, rcc_mock):
        changed.global_()
        rcc_mock.assert_called_with('global_') 

    @patch('srv.modules.runners.changed.requires_conf_change')
    def test_client(self, rcc_mock):
        changed.client()
        rcc_mock.assert_called_with('client') 
