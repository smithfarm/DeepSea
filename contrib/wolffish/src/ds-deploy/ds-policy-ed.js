...
Polymer({
    ...
    ready: function() {
        ...
        this.getClusterProposal();
    },

    // Generates some phony data  now.
    // TODO: Remove this once we have a runner a DeepSea.
        ...
        for (profile in profiles) {
            ...
            for (var i = 0; i < profiles[profile].length; i++) {
                ...
                if (minion) {
                    ...
                    // ERROR: We trust DeepSea, so we should not get here.
    ...
    _parseClusterProposal: function(cluster) {
        ...
        if (!cluster.minions || !cluster.roles || !cluster.profiles) {
            console.error(this.is + ": No cluster proposal data received.");
            alert("Failed to receive cluster proposal data.  DeepSea discovery stage not run?");
        } else {
            // Populate minions.
            ret = this._parseClusterMinions(cluster.minions);


            ret = this._parseClusterRoles(cluster.roles);


            ret = this._parseClusterProfiles(cluster.profiles);
    ...
    getClusterProposal: function() {
        ...
        // Parse the cluster JSON object.
        this._parseClusterProposal(cluster);
    }
});
