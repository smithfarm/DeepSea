...
function zypper_ref {
    ...
    for delay in 60 60 60 60 ; do
        zypper --non-interactive --no-gpg-checks refresh && break
        sleep $delay
    done
    set -x
}
...
function run_stage_2 {
  salt '*' cmd.run "for delay in 60 60 60 60 ; do zypper --non-interactive --no-gpg-checks refresh && break ; sleep $delay ; done"
  _run_stage 2 "$@"
  salt_pillar_items
}
